...
export async function QuerySmartStakeAPI(appName, days) {
    let URL = `https://ssdata.smartstakeapi.com/common/valperf?app=${appName}&accessKey=${SMARTSTAKE_ACCESS_KEY}&numberOfDays=${days}`
    const resp = await fetch(URL)
    const data = await resp.json()
    if (data.length === undefined || data.errors !== undefined) {
        throw new Error(`SmartStakeAPI returned data with zero length, data: ${JSON.stringify(data)}`)
    }
    return data
}
